							...
							<div class="ml-4 flex-1 flex flex-col justify-between sm:ml-6">
								<div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
									<div>
										...
										<p class="mt-1 text-sm font-medium text-gray-900">{{ getCurrency(product.price) }}</p>
									</div>
				...
				<section aria-labelledby="summary-heading"
					...
					<dl class="mt-6 space-y-4">
						<div class="flex items-center justify-between">
							...
							<dd class="text-sm font-medium text-gray-900">{{getCurrency(calcPrice())}}</dd>
						</div>
						<div class="border-t border-gray-200 pt-4 flex items-center justify-between" v-if="shipping">
							...
							<dd class="text-sm font-medium text-gray-900">{{getCurrency(shipping)}}</dd>
						</div>
						<div class="border-t border-gray-200 pt-4 flex items-center justify-between" v-if="tax">
							...
							<dd class="text-sm font-medium text-gray-900">{{getCurrency(tax)}}</dd>
						</div>
...
export default {
    ...
    methods: {
        calcTotal() {
            return this.getCurrency(this.calcPrice()*1 + this.shipping*1 + this.tax*1);
        },
        calcPrice() {
            let sum = 0;
            if (this.productList) {
