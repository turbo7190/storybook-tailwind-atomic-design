				...
				<div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
					...
					<div class="px-5 py-5 bg-gray-50 space-y-6 sm:flex sm:space-y-0 sm:space-x-10 sm:px-8">
						<div v-for="item in list2" :key="item.name" class="flow-root">
							<a :href="item.href"
								 class="-m-3 p-3 flex items-center rounded-md hover:bg-gray-100 transition ease-in-out duration-150">
								<Icon :name="item.icon" isOutline size="6" color="gray-400" class="flex-shrink-0"/>
								<Text size="base" weight="medium" color="gray-900" class="mt-1 ml-2" :content="item.name"/>
							</a>
						</div>
					</div>
				</div>
...
/**
 * - Use it to show a stacked flyout menu with footer actions
 */
export default {
	components: {
		Popover,
