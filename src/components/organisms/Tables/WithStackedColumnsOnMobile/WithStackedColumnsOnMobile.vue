		...
		<div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
			<table class="min-w-full divide-y divide-gray-300">
				...
				<tr v-for="(item, id) in list" :key="id">
					<td v-for="(i, index) in headers" :key="index" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
						{{item && item[i.name]}}
						<dl class="font-normal lg:hidden" v-if="i.name === 'name'">
							<dd class="mt-1 truncate text-gray-700">{{ item.title }}</dd>
...
/**
 * - Use it to show a simple table with stacked columns on mobile
 */
export default {
	components: {
		Button, Link, Text
